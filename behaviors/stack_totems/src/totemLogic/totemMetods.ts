...
import { beforeEventsSimplified } from "simplified-mojang-api";
...
class TotemsMetods {
    ...
    private onUseTotem(): void {
        beforeEventsSimplified.onEntityHeal((args) => {
            const { healSource: source, healedEntity: entity } = args;

            if (!(entity instanceof mc.Player)) return;
            if (source.cause != mc.EntityHealCause.TotemOfUndying) return;
        });
